...
class KnownTopicsOverviewTest extends TestCase
{
    /** @test */
    public function retrieve_all_data()
    {
        $response = $this
            ->getJson('/logs/topics/known', ['Authorization' => "Bearer {$this->adminToken()}"])
            ->assertStatus(200)
            ->assertJsonStructure([
                'code',
                'message',
        ...
        $this->assertEquals(ErrorTopic::where('known', true)->get()->count(), count($content['data']));
